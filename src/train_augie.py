...
import argparse
import signal
import Optimal_Stop # my test env
import pickle
...
def EVaR(rewards, threshold):
    ...
    Takes as inputs discounted sum of rewards and alpha.
    ----TODO-------
    Idea incorporate the natural gradient with risk metric look at paper"""
...
def main(env_name, num_episodes, gamma, lam, kl_targ, batch_size, hid1_mult, policy_logvar, print_results, risk_targ):
    ...
    while episode < num_episodes:
        ...
        nodisc0 = -0.0001*np.array([t['rewards'].sum() for t in trajectories]) # scaled for gradients
        print(nodisc0)
        ...
        lamb = policy.update(observes, actions, advantages, logger)  # update policy
